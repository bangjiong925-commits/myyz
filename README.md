...
- **远程数据库**: 使用MongoDB Atlas云数据库存储，支持高可用性和扩展性
- **实时展示**: Web界面实时从远程数据库显示开奖数据和统计信息
- **API服务**: 提供RESTful API接口，所有数据来源于远程MongoDB
- **智能调度**: 支持定时任务和内置调度器
- **健康监控**: 完整的系统状态监控和日志记录

### 数据功能
- **开奖记录**: 完整的开奖号码、时间记录，存储在远程MongoDB Atlas
- **统计分析**: 号码频率、遗漏值、热冷分析，实时从数据库计算
- **数据验证**: 期号连续性检查和数据完整性验证
- **历史数据**: 支持历史数据查询和导出，无本地文件依赖
...
本系统已完全迁移到**远程MongoDB Atlas数据库架构**，具有以下特点：

- **云端存储**: 所有数据存储在MongoDB Atlas云数据库中
- **无本地依赖**: 前端页面和API接口完全从远程数据库读取数据
- **高可用性**: 利用MongoDB Atlas的集群和备份功能
- **实时同步**: 数据抓取后立即存储到云数据库，前端实时显示
- **扩展性强**: 支持多实例部署，共享同一数据源

### 数据流程

```
数据源网站 → 爬虫脚本 → MongoDB Atlas → API服务器 → Web前端
     ↓              ↓           ↓            ↓         ↓
  实时开奖数据    自动抓取    云端存储    RESTful API  实时展示
```
...
```bash
# MongoDB Atlas连接配置
export MONGODB_URI="mongodb+srv://username:password@cluster.mongodb.net/taiwan_pk10?retryWrites=true&w=majority"
